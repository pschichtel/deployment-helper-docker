...
RUN apk add --update --no-cache bash jq python3 docker-cli docker-compose git curl openssh vim tcpdump ca-certificates coreutils grep sed gettext socat openjdk17-jre-headless helm strace
RUN apk add --no-cache -X http://dl-cdn.alpinelinux.org/alpine/edge/community kubectl podman fuse-overlayfs
